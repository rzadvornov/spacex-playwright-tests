...
export class MediaCarouselSteps {
  ...
  async viewMediaCarousel() {
    await this.verifyCarouselVisibility();
  }
  ...
  async viewMediaCarouselWithTileActive(expectedTile: string) {
    ...
    await this.verifyTileIsActive(expectedTile);
  }
  ...
  private async verifyCarouselVisibility(): Promise<void> {
  ...
  private async verifyTileIsActive(expectedTile: string): Promise<void> {
  ...
  async checkMediaTiles(dataTable: DataTable) {
    ...
    this.verifyTileCount(tiles.length);
    await this.validateAllTiles(tiles, expectedTiles);
  }

  private verifyTileCount(actualCount: number): void {
    expect(
      actualCount,
  ...
  private async validateAllTiles(
    tiles: any[],
    expectedTiles: MediaTile[]
  ): Promise<void> {
  ...
  @Then("the carousel should go back to the {string} tile")
  async checkCarouselWentBack(expectedTile: string) {
    await this.verifyTileIsActive(expectedTile);
  }
  ...
  async checkVideoOverlayOpen() {
    await this.verifyVideoOverlayVisible();
  }

  @Then("the video overlay should open")
  async checkVideoOverlayOpened() {
    await this.verifyVideoOverlayVisible();
  }

  private async verifyVideoOverlayVisible(): Promise<void> {
    const isVisible =
      await this.humanSpaceflightPage.mediaCarousel.isVideoOverlayVisible();
    expect(isVisible, "Video overlay should open").toBeTruthy();
  }
  ...
  async checkOverlayClosed() {
    await this.verifyOverlayClosed();
  }
  ...
  private async verifyOverlayClosed(): Promise<void> {
