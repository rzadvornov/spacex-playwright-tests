...
import { ElementAccessibilityInfo } from "../types/ElementAccessibilityInfo";
import { MediaAccessibilityInfo } from "../types/MediaAccessibilityInfo";
...
export class AccessibilityPOF {
  ...
  public async getHeadingCounts(): Promise<Record<string, number>> {
    const headings = await this.page.locator("h1:visible, h2:visible, h3:visible, h4:visible, h5:visible, h6:visible").all();
  ...
  public async checkHeadingHierarchy(): Promise<boolean> {
    ...
    for (const heading of headings) {
      const tagName = await heading.evaluate((el: Element) => el.tagName.toLowerCase());
  ...
  private async checkElementFocusability(element: Locator): Promise<boolean> {
    return await element.evaluate((el: Element) => {
      ...
      const nativeFocusableTags = ['a', 'button', 'input', 'select', 'textarea', 'details'];
      const hasValidHref = tagName === 'a' && (el as HTMLAnchorElement).href !== '';
      
      return nativeFocusableTags.includes(tagName) || hasValidHref || tabIndex >= 0;
    });
  }
  ...
  private async checkFocusIndicator(element: Locator): Promise<boolean> {
    return await element.evaluate((el: Element) => {
      ...
      const hasBorderChange = parseFloat(style.borderWidth) > 0 && style.borderColor !== 'transparent';
  ...
  private async checkElementOperability(element: Locator): Promise<boolean> {
    return await element.evaluate((el: Element) => {
      ...
      const hasInteractiveRole = ['button', 'link', 'checkbox', 'radio'].includes(el.getAttribute('role') || '');
  ...
  public async checkDescriptiveLinkText(): Promise<{ nonDescriptiveLinkCount: number }> {
    const links = await this.page.locator('a:visible, [role="button"]:visible').all();
    const genericTexts = new Set(['click here', 'read more', 'learn more', 'go', 'view', 'here', 'link']);
  ...
  public async checkMediaFeatures(mediaType: "video" | "audio"): Promise<MediaAccessibilityInfo> {
    const selector = mediaType === "video" ? "video:visible" : "audio:visible";
    const mediaElements = await this.page.locator(selector).all();
    
    if (mediaElements.length === 0) {
      return { hasControls: true, hasCaptions: true, isKeyboardAccessible: true };
    }

    const results = await Promise.all(
      mediaElements.map(async (media) => {
        const hasControls = await media.evaluate((el: HTMLMediaElement) => el.controls);
        const hasCaptions = mediaType === "video" ? 
          (await media.locator('track[kind="captions"]').count()) > 0 : true;
  ...
  public async checkMotionAccessibility(): Promise<{ 
    ...
    const [noAutoAdvance, noParallax, hasAggressiveAnimations] = await Promise.all([
  ...
  private async checkAutoAdvanceContent(): Promise<boolean> {
    return await this.page.evaluate(() => {
      ...
      return !autoAdvanceSelectors.some(selector => document.querySelector(selector));
    });
  }

  private async checkParallaxEffects(): Promise<boolean> {
    return await this.page.evaluate(() => {
      const parallaxSelectors = ['.parallax', '[data-parallax]', '[style*="parallax"]'];
      return !parallaxSelectors.some(selector => document.querySelector(selector));
    });
  }
  ...
  private async checkFlashingCSSClasses(): Promise<boolean> {
    return await this.page.evaluate(() => {
      const flashingSelectors = ['.blink', '.flash', '.flicker', '[class*="blink"]', '[class*="flash"]'];
      return flashingSelectors.some(selector => document.querySelector(selector));
    });
  }
  ...
  public async submitFormWithInvalidData(formSelector: string = 'form'): Promise<void> {
